...
function formatGoogleDateTime(dateValue, timeValue, hoursToAdd = 0) {
    ...
    const date = new Date(Date.UTC(year, month - 1, day, hour + hoursToAdd, minute || 0));
...
router.get('/google-calendar', async function(req, res){
    try {
        ...
        return res.redirect(`https://calendar.google.com/calendar/render?${calendarParams.toString()}`);
    } catch (err) {
